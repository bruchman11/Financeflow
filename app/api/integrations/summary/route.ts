...
import { getSummary } from "@/lib/db/integrations";
import { json, errorResponse, resolveRange } from "@/lib/integrations/http";

/** Entradas/saídas/saldo do período (padrão: mês corrente). */
export async function GET(req: NextRequest) {
  ...
  try {
    const summary = await getSummary(auth.contact.companyId, from, to);
    return json(200, { ok: true, from, to, ...summary });
  } catch (e) {
